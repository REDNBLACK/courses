...
class TrackFlatSpec extends FlatSpec with MustMatchers {
  ...
  it should """have a constructor that accepts the name and the length of the track
    ...
    track.period must be(Duration.ofHours(5).withSeconds(0))
  }

  it must """throw an IllegalArgumentException with the message
    Track name cannot be blank if the name of the track is blank.""" in {
    val exception = the [IllegalArgumentException] thrownBy new Track("")
